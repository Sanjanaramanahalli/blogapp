...
function authenticate(req, res, next) {
  ...
  try {
    ...
    const user = db.prepare('SELECT id, name, email, role, avatar_url, bio, auth_provider, google_id, linkedin_id, github_id, created_at FROM users WHERE id = ?').get(decoded.id);
    req.user = user || null;
  } catch (err) {
    req.user = null;
  }
